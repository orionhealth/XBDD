...
YUI.add('gallery-lightbox', function (Y) {
	...
	Y.extend(LB, Y.Base, {
		...
		initializer: function () {
				...
				.append(create('<div id="imageDataContainer"></div>')
					.append(create('<div id="imageData"></div>')
						...
						.append(create('<div id="bottomNav"></div>')
							.append(create('<a id="bottomNavClose" href="#" class="glyphicon glyphicon-remove"></a>'))
						)
					)
				)
			);
			...
			Y.one("#overlay").hide().on(CLICK, function () {
				this.end();
			}, this);
			...
			Y.one("#outerImageContainer").setStyles({
				width: size,
				height: size
			});
			...
			Y.one("#bottomNavClose").on(CLICK, function (evt) {
				evt.halt();
				this.end();
			}, this);
			...
			L.later(0, this, function () {
				...
				Y.Array.each(ids.split(" "), function (element) {
		...
		start: function (selectedLink) {
			...
			var overlay = this.get(OVERLAY).setStyles({
				height: Y.DOM.docHeight() * 2 + PX,
			...
			if (this.get(ANIM)) {
				var anim = new Y.Anim({
					...
					to: {
						opacity: this.get(OVERLAY_OPACITY)
					},
		...
		_resizeImageContainer: function (imgWidth, imgHeight) {
			...
			var outerImageContainer = this.get("outerImageContainer"),
				...
				afterResize = Y.bind(function () {
					this.get(PREV_LINK).setStyles({
						height: imgHeight + PX
					});
					this.get(NEXT_LINK).setStyles({
						height: imgHeight + PX
					});
		...
		_updateDetails: function () {
			...
			if (this.get(ANIM)) {
				...
				anim.on("end", function () {
					...
					this.get(OVERLAY).setStyle("height", Y.DOM.docHeight() * 2 + PX);
					this._updateNav();
				}, this);
		...
		/**
		 * Disable Scrolling on the page when opening Lightbox.
		 *
		 * @method _disableScrollNav
		 * @private
		 */
		_disableScrollNav: function () {
			Y.one(document.body).addClass("noscroll"); //better if we can keep scroll bar visible
		},

		/**
		 * Enable scrolling when closing Lightbox.
		 *
		 * @method _enableScrollNav
		 * @private
		 */
		_enableScrollNav: function () {
